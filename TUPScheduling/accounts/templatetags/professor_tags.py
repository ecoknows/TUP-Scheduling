...
@register.inclusion_tag("account/includes/result_list.html",
                        takes_context=True)
...
@register.inclusion_tag(
    "account/includes/result_row.html", takes_context=True)
def result_row_display(context, index):
    ...
    view_buttons = view.get_buttons_for_obj(obj)
    view_buttons.append(
        {'url': '/delete-prof-schedule/'+ str(obj.pk), 'label': 'Reset Schedule', 'classname': 'button no button-small button-secondary', 'title': 'Reset Schedule Professor'}
    )
    
    context.update({
        'obj': obj,
        'row_attrs': mark_safe(flatatt(row_attrs_dict)),
        'action_buttons': view_buttons,
    })
    return context


@register.inclusion_tag(
    "account/includes/result_row_value.html", takes_context=True)
